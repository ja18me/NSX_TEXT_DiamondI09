...
def SaveToFile(Energy,Ji0,Data,RunName):
    with open(directoryName+"/"+RunName[0:-4]+".txt","w") as txt_file:
        ...
        for i in range(len(Energy)):
            txt_file.write("".join(str(Energy[i]*1000))+"\t"+"".join(str(Ji0[i]))+"\t"+"".join(str(Data[i]))+"\t"+"".join(str(Data[i]/Ji0[i]))+"\n")
...
for x in files:
    if x[-3:]!='nxs':
        ...
        dataSDC=a['/entry1/smpmamp39/smpmamp39'].nxdata
        energy=a['/entry1/instrument/jenergy/jenergy'].nxdata
        dataJI0=a['/entry1/sm5amp8/sm5amp8'].nxdata
